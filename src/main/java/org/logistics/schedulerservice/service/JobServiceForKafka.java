...
import org.logistics.schedulerservice.quartzJob.KafkaJob;
import org.quartz.*;
...
@Slf4j
@Service
@Qualifier("kafkaJobService")
public class JobServiceForKafka implements JobService {

    private JobScheduler jobScheduler;
    private ValidatorFactory validatorFactory;
    private Validator validator;

    public JobServiceForKafka(JobScheduler jobScheduler) {
    ...
    public ScheduleResponse createSchedule(CreateScheduleRequest createScheduleRequest) {
        ...
        try {
            KafkaRequestModel kafkaRequestModel = validateAndReturnConvertedJobDataFromObject(createScheduleRequest.getJobData());
            JobDetail jobDetail = generateJobDetails(kafkaRequestModel);
            ...
            scheduleResponse.setMessage("kafka job scheduled successfully");
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    public ScheduleResponse updateSchedule(UpdateScheduleRequest updateScheduleRequest) {
        ...
        try {
            ...
            updateScheduleResponse.setMessage("kafka job has been re-scheduled successfully");
        } catch (Exception ex) {
    ...
    public ScheduleResponse deleteSchedule(String jobId) {
        ...
        try {
            Trigger jobTrigger = getTriggerDetail(jobId, "kafka-jobs");
            if( jobScheduler.getScheduler().unscheduleJob(jobTrigger.getKey()) &&
                    jobScheduler.getScheduler().deleteJob(new JobKey(jobId, "kafka-jobs"))) {
                deleteScheduleResponse.setSuccess(true);
                deleteScheduleResponse.setMessage("kafka job has been deleted successfully");
            } else {
                deleteScheduleResponse.setSuccess(false);
                deleteScheduleResponse.setMessage("kafka job removal activity failed");
            }
        }  catch (Exception ex) {
            deleteScheduleResponse.setSuccess(false);
    ...
    private KafkaRequestModel validateAndReturnConvertedJobDataFromObject(String jobData) throws IOException {
        KafkaRequestModel kafkaRequestModel = null;
        ObjectMapper mapper = new ObjectMapper();
        ObjectReader objectReader = mapper.readerFor(KafkaRequestModel.class);
        objectReader.without(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES);
        kafkaRequestModel = objectReader.readValue(jobData, KafkaRequestModel.class);
        Set<ConstraintViolation<KafkaRequestModel>> violations = validator.validate(kafkaRequestModel);
        if (violations.size() != 0) {
            for (ConstraintViolation<KafkaRequestModel> violation : violations)
                log.error(violation.getMessage());
            log.warn("skipping the current message due to contract violation");
            ArrayList<String> errors = new ArrayList<>();
            violations.forEach(kafkaRequestModelConstraintViolation -> {errors.add(kafkaRequestModelConstraintViolation.getMessage());});
            throw new SchedulerServiceException(String.join(", ", errors));
        }
        return kafkaRequestModel;
    }
    ...
    private Trigger createTrigger(RequestTriggerDetails requestTriggerDetails, JobDetail jobDetail) {
        ...
        if (requestTriggerDetails.getCronExpression() == null)
            ...
            return TriggerBuilder.newTrigger()
                    .withIdentity(UUID.randomUUID().toString(), "kafkaTrigger")
    ...
    private JobDetail generateJobDetails(KafkaRequestModel kafkaRequestModel) {
        log.info("Inside : {}", new Object() {
        }.getClass().getEnclosingMethod().getName());
        log.info("creating job details object");
        return JobBuilder.newJob(KafkaJob.class)
                .withIdentity(UUID.randomUUID().toString(), "kafka-jobs")
                .withDescription("kafka job scheduling details")
                .usingJobData(generateKafkaJobDataMap(kafkaRequestModel))
                .storeDurably()
                .requestRecovery(true)
                .build();
    }

    private JobDataMap generateKafkaJobDataMap(KafkaRequestModel kafkaRequestModel) {
        log.info("Inside : {}", new Object() {
        }.getClass().getEnclosingMethod().getName());
        JobDataMap jobDataMap = new JobDataMap();
        jobDataMap.put("eventType", kafkaRequestModel.getEventType());
        jobDataMap.put("eventData", kafkaRequestModel.getEventData());
        jobDataMap.put("messageKey", kafkaRequestModel.getMessageKey());
        return jobDataMap;
    }
